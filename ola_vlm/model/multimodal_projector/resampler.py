...
class TaskTokenResampler(nn.Module):
    ...
    def __init__(
        ...
        # self.latents = nn.Parameter(torch.randn(1, num_queries, dim) / dim**0.5)
        # self.task_w = nn.Parameter(torch.tensor(0.0))
        self.proj_in = nn.Linear(embedding_dim, dim)
    ...
    def forward(self, x, latents):
        ...
        if latents is None:
            latents = torch.zeros(x.size[0], self.num_queries, self.dim)
        else:
            if latents.shape[1] != self.num_queries:
                if self.num_queries > 1 and self.num_queries % latents.shape[1] == 0:
                    n = latents.shape[1]
                    latents = latents.repeat(1, self.num_queries // n, 1)
                else:
                    latents = latents.mean(dim=1, keepdim=True).repeat(1, self.num_queries, 1)
