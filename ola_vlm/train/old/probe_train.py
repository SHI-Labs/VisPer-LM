...
class ModelArguments:
    ...
    # probe
    image_generator: Optional[str] = field(default="runwayml/stable-diffusion-v1-5")
    probe_depth: Optional[int] = 1
    probe_dim_head: Optional[int] = 32
    probe_num_heads: Optional[int] = 4
    probe_num_tokens: Optional[int] = 77
    probe_output_dim: Optional[int] = 768
    probe_ff_mult: Optional[int] = 1
...
def prepare_coco(json_file):
    ...
    for annotation in tqdm(coco_data["annotations"]):
        ...
        question = "Describe the image in two lines.\n<image>"
        conversations = [
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        list_data_dict = prepare_coco(data_path)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in self.list_data_dict[i]:
            ...
            data_dict["caption"] =  self.list_data_dict[i]['caption']
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        if 'phi' in model_args.model_name_or_path.lower():
            model = SherlockProbeLlavaPhi3ForCausalLM.from_pretrained(
            ...
            model = SherlockProbeLlavaLlamaForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                attn_implementation=attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
    ...
    model.config.image_gen = {
        "probe_depth": model_args.probe_depth,
        "probe_dim_head": model_args.probe_dim_head,
        "probe_num_heads": model_args.probe_num_heads,
        "probe_num_tokens": model_args.probe_num_tokens,
        "probe_output_dim": model_args.probe_output_dim,
        "probe_ff_mult": model_args.probe_ff_mult,
    }
    model.config.image_generator = model_args.image_generator

    model.init_probes(model.config)
    model.init_image_generator(model.config)
    ...
    for r in model.resamplers:
        for p in r.parameters():
            p.requires_grad = True
    
    try:
        for r in model.img_resamplers:
            for p in r.parameters():
                p.requires_grad = True
    except:
        pass
