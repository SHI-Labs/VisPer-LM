...
class ModelArguments:
    ...
    freeze_task_token: Optional[bool] = field(default=True)
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        if model_args.unfreeze_whole_model:
    ...
    if hasattr(model.config, "task_token_format"):
        if model.config.num_task_tokens > 0:
            requires_grad_value = not model_args.freeze_task_token
            if "gen" in model.config.aux_mode:
                model.get_model().special_gen_tokens.requires_grad_(requires_grad_value)
            if "seg" in model.config.aux_mode:
                model.get_model().special_seg_tokens.requires_grad_(requires_grad_value)
            if "depth" in model.config.aux_mode:
                model.get_model().special_depth_tokens.requires_grad_(requires_grad_value)
