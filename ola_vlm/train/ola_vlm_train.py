...
class ModelArguments:
    ...
    contrastive_loss_weight: Optional[float] = field(default=0.3)
    ...
    mode: Optional[str] = field(default="gen-depth-seg")
    num_task_tokens: Optional[int] = 8
    pass_text_to_aux: Optional[bool] = True
    
    use_contrastive: Optional[bool] = True
    layer_indices: Optional[str] = "d8-20_s10-18_g12-20"
    ...
    seg_teacher: Optional[str] = "oneformer"
...
def train(attn_implementation=None):
    ...
    if "ola" in model_args.model_name_or_path:
    ...
    if "ola" not in model_args.model_name_or_path.split("/")[-1]:
    ...
    for name, p in model.named_parameters():
        if "da_v2_head." in name or "gen_encoder." in name or "dav2_backbone." in name or "oneformer." in name:
            p.requires_grad = False


    if model_args.num_task_tokens > 0:
        if model_args.freeze_task_token:
