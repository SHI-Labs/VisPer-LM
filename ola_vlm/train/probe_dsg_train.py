...
from ola_vlm.mm_utils import process_highres_image, process_anyres_image, process_highres_image_crop_split, tokenizer_image_token
...
class ModelArguments:
    ...
    mode: Optional[str] = field(default="seg")
    ...
    seg_head_output_dim: Optional[int] = 1536
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        model = ProbeDSGLlavaLlamaForCausalLM.from_pretrained(
    ...
    data_args.mode = model_args.mode

    data_args.image_aspect_ratio = model.config.image_aspect_ratio
    ...
    model.config.probe_mode = model_args.mode

    model.requires_grad_(False)

    if model.config.probe_mode == "gen":
    ...
    elif model.config.probe_mode == "seg":
    ...
    elif model.config.probe_mode == "depth":
        ...
        for n, p in model.named_parameters():
            if p.requires_grad:
                if "gen_head" in n:
                    ...
                    seg_heads += p.numel()
                print(n)
        print(depth_heads, gen_heads, seg_heads)
